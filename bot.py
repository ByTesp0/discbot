...
# ==================== 2. НАСТРОЙКИ ====================
ROLE_ID_TO_TRACK = int(os.getenv("1470909799502712935", "0"))  # ID роли, которую нужно отслеживать
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if ROLE_ID_TO_TRACK in after_roles and ROLE_ID_TO_TRACK not in before_roles:
            ...
            async for entry in after.guild.audit_logs(limit=5, action=discord.AuditLogAction.member_role_update):
                if entry.target.id == after.id and any(r.id == ROLE_ID_TO_TRACK for r in entry.after.roles):
...
def main():
    ...
    if not TOKEN:
        logger.error("❌ Не найдена переменная окружения DISCORD_TOKEN")
        sys.exit(1)
